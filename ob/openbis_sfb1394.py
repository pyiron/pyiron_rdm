...
def GenericLammpsJobObject(o, space, project, collection, concept_dict, show_object=True):
    ...
    if exists:
        ...
        if 'molecular_statics' in concept_dict.keys() and 'minimization_algorithm' in cdict.keys():
            ...
            props_dict['description_multiline'] = description
        ...
        parents_to_link = {} # TODO: discuss with Khalil
        ...
        upload_pyiron_job_h5(o, object_, ob_coll, cdict, h5_file_path, kind)
        upload_environment_yml(o, object_, ob_coll, cdict, yml_file_path, kind)
        upload_concept_dict(o, object_, cdict_file_name, ob_coll, cdict_file_path, kind)
...
def MurnaghanJobObject(o, space, project, collection, concept_dict, children_concept_dict, show_object=True):
    # TODO: SFB instance does not have this object type yet
    # o = openbis_login(url, user)
...
def GenericCrystalObject(o, space, project, collection, concept_dict, show_object=True):
    # TODO: discuss SFB strategy on structures
    # o = openbis_login(url, user)
    ...
    if exists == True:
        ...
        upload_atomistic_structure_h5(o, object_, struct_file_name, ob_coll, struct_file_path, kind)
...
def upload_pyiron_job_h5(o, job_object, collection, cdict, file_path, kind):
    from datetime import datetime

    dataset_props_dict = {'$name': cdict['job_name'] + '.h5'}
    ds_type = 'PYIRON_HDF5'
    
    ds_hdf = o.new_dataset(
        type       = ds_type,
        collection = collection,
        object     = job_object.identifier,
        files      = [file_path],
        kind       = kind,
...
def upload_environment_yml(o, job_object, collection, cdict, file_path, kind):
    dataset_props_dict = {'$name':cdict['job_name']+'_environment.yml'}
    ds_type = 'ATTACHMENT' # TODO: check with Khalil whether to upload
    try:
...
# TODO: delete if structure not uploaded?
def upload_atomistic_structure_h5(o, structure_object, structure_name, collection, file_path, kind):
    dataset_props_dict = {
        '$name': structure_name,
    ...
    try:
        ds_hdf = o.new_dataset(
            ...
            collection = collection,
            object     = structure_object.identifier,
            files      = [file_path],
            kind       = kind,
            props      = dataset_props_dict
        )
...
def upload_concept_dict(o, ob_object, cdict_name, collection, file_path, kind):
    try:
        ds_json = o.new_dataset(
            type       = 'ATTACHMENT',
            collection = collection,
            object     = ob_object.identifier,
            files      = [file_path],
            kind       = kind,
